...
func TestAuditAfterRecordDurable(t *testing.T) {
	dir := t.TempDir()
	...
	if err := cl.Complete("ns-01", "t1", "exec", "failed"); err == nil {
		t.Fatal("complete must fail when the terminal record is not durable")
	}
	if n := len(cl.Audit.FilterByType("complete")); n != 0 {
		t.Fatalf("audit must not report completion before the record is durable, got %d events", n)
	}
}
